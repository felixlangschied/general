...
gff_path = '/share/project/felixl/ncOrtho/data/mirGeneDB/data/human.gff'
pre_path = '/share/project/felixl/ncOrtho/data/mirGeneDB/data/human_precursor.fa'
...
with open(pre_path, 'r') as fh:
    for line in fh:
        if line.startswith('>'):
            header = line.strip().replace('>', '').replace('_pre', '')
        else:
            seq = line.strip()
            seq_dict[header] = seq
